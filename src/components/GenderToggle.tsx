...
const GenderToggle = () => {
  ...
  return (
    <div className="absolute  right-0  flex  items-center  p-2">
      <label htmlFor="genderToggle" className="inline">Male</label>

      <div>
        <input type="checkbox" className="hidden" id="genderToggle" onChange={(e) => setGender(e.target.checked ? 'female' : 'male')} />

        <label htmlFor="genderToggle" className="block  w-12   h-6  mx-2  bg-gray-800  rounded-full  transition-colors">
